...
namespace DraxTechnology
{
    ...
    // NWM DATA STRUCT
    {
        ...
        // These are serialised by RenderBytes() into the AMX wire layout and are
        // intentionally always 0 today (events use OurEvent, not the node/zone/op
        // path). Explicit = 0 keeps the byte layout identical while clearing CS0649.
        public int Node = 0;    // Node number	- set event to 0 if using this [RPJ - __int16]
        public int Zone = 0;    // Zone number	- set event to 0 if using this [RPJ - __int16]
        public int Op = 0;  // Output number - set event to 0 if using this [RPJ - __int16]
        public int ControlType = 0; // Control type	- set event to 0 if using this [RPJ - __int16]
        public int Dat1 = 0;    // Data 1		- set event to 0 if using this used [RPJ - long]
        public int Dat2 = 0;    // Data 2		- set event to 0 if using this used [RPJ - long]
        public int Dat3 = 0;    // Data 3		- set event to 0 if using this used [RPJ - long]
        public int Dat4 = 0;    // Data 4		- set event to 0 if using this used [RPJ - long]
        public int Dat5 = 0;    // Data 5		- set event to 0 if using this used [RPJ - long]
        public int Dat6 = 0;    // Data 6		- set event to 0 if using this used [RPJ - long]
