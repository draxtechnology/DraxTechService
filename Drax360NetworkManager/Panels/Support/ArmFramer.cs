...
namespace DraxTechnology.Panels
{
    internal class ArmFramer
    {
        ...
        public ArmFramer(Action<byte[], int, int> writeAction)
        {
            _write = writeAction ?? throw new ArgumentNullException(nameof(writeAction));
        }

        public ArmFramer(Action<byte[]> writeAction)
            : this((buf, off, len) =>
            {
        ...
        private void ProcessCompleteFrame()
        {
            ...
            if (record != null)
            {
                LogMsg("ARM addr=" + record.PagerAddress + " | " + record.DisplayText);
        ...
        private void LogMsg(string msg) => Log?.Invoke("[ArmFramer] " + msg);
    }
}
